...
if (typeof window !== 'undefined' && typeof window.ethereum !== 'undefined') {
  ...
  console.log('index.js loaded but without access to windows.ethereum');
}
...
async function queryMetamaskStatus() {
  ...
  if (typeof window !== 'undefined' && typeof window.ethereum !== 'undefined') {
    ...
    if (browser === 'Chrome') {
      ...
      status +=
        "<a href='https://chrome.google.com/webstore/detail/metamask/nkbihfbeogaeaoehlefnkodbefgpgknn'>Install it</a> or check extension settings to enable it.";
      statustype = 'alert-warning';
    } else if (browser === 'Firefox') {
      status = "MetaMask is not installed or it's disabled!<hr>";
      status +=
        "<a href='https://addons.mozilla.org/fr/firefox/addon/ether-metamask'>Install it</a>  or check extension settings to enable it.";
      statustype = 'alert-warning';
    } else {
      status = 'MetaMask is not installed!<hr>';
      status +=
        "It seem your're using " + browser + ' browser. Metamask extension exists only for Chrome and Firefox browser.';
      statustype = 'alert-danger';
    }
  }
...
function updateUI() {
  // used browser
  document.getElementById('detectedbrowser').innerHTML =
    'This page is loaded on <strong>' + detectBrowser() + '</strong> browser.';
